...
namespace NEbml.MkvTitleEdit.Tests
{
	...
	{
		[TestCase(@"C:\Users\azurestef\Downloads\Estas Tonne - Internal Flight Experience (Live in Cluj Napoca).webm"), Explicit]
		public void MkvInfo(string filePath)
		{
			using (var dataStream = File.OpenRead(filePath))
			{
				...
				var infoDumper = MakeElementDumper(
					MatroskaDtd.Segment.Info.Duration
					);
				...
				if (reader.LocateElement(MatroskaDtd.Segment))
				{
					...
					if (reader.LocateElement(MatroskaDtd.Segment.Info))
					{
						...
						{
							Console.WriteLine(reader.ElementId);
							//if (reader.ElementId == MatroskaDtd.Segment.Info.Duration.Identifier)
							{
								infoDumper(reader);
							}
				...
				//https://gitlab.com/nitroxis/matroska
